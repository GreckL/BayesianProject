...
for (i in 1:s) {      # Number of simulations
    # Generator process (study-dependent)
    for (t in 1:nstud) {
    ...
    bg <- baggr(df_pooled, model="rubin", pooling = "partial",iter=6000)
    ...
    BAY_MSE <- sum(unlist(theta_MSE))/nstud
    MSE_hat = MSE_hat*((s-1)/s) + BAY_MSE*(1/s) #Average over simulation
BigP <- heterogeneity(bg)$mean
...
## Can include average heterogeneity (pooling) in loop



############ Graphs

bg2 <- baggr(df_pooled, model="rubin", pooling = "full")
bg3 <- baggr(df_pooled, model="rubin", pooling = "none")

baggr_comparison <- baggr_compare("Partial pooling" = bg, 
                                  "Full pooling" = bg2,
                                  "No pooling" = bg3, 
                                  plot = TRUE) 
dev.print(pdf, 'Model_comp_RE_n20.pdf', width=8, height=5)



PriorP1 <- baggr(df_pooled,ppd=TRUE)
PriorP2 <- baggr(schools, prior_hypermean = normal(1, 300), ppd=TRUE)
